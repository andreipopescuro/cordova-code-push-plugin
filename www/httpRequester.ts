...
class HttpRequester implements Http.Requester {
  constructor(contentType?: string) {
    ...
    cordova.plugin.http.setHeader(
      ...
      cordova.require("cordova/plugin_list").metadata["code-push"]
    );
    if (contentType) {
   ...
   */
  private static getInitialOptionsForVerb(
    verb: Http.Verb
  ): AdvancedHttp.Options | Error {
    switch (verb) {
      case Http.Verb.GET:
        return { method: "get" };
      ...
      default:
        return new (class UnsupportedMethodError extends Error {})(
          `Unsupported HTTP method code [${verb}]`
        );
    }
  }
}
